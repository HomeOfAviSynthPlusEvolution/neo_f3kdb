...
static const int parallel_width = 16;
...
static __forceinline void process_plane_info_block(
    ...
    const __m256i &src_pitch_vector, 
    const __m256i &minus_one, 
    ...
    char*& info_data_stream)
{
    ...
    __m256i info_block = _mm256_load_si256((__m256i*)info_ptr);

    // ref1: bit 0-7
    // left-shift & right-shift 24bits to remove other elements and preserve sign
    __m256i ref1 = info_block;
    ref1 = _mm256_slli_epi32(ref1, 24); // << 24
    ref1 = _mm256_srai_epi32(ref1, 24); // >> 24
    ...
    case 0:
        ...
        temp_ref1 = _mm256_abs_epi32(ref1);
        temp_ref1 = _mm256_sra_epi32(temp_ref1, height_subsample_vector);
        temp_ref1 = _mm256_mullo_epi32(temp_ref1, _mm256_srai_epi32(ref1, 31));
        ref_offset1 = _mm256_mullo_epi32(src_pitch_vector, temp_ref1); // packed DWORD multiplication
        break;
    case 1:
        // ref1 is guarenteed to be postive
        temp_ref1 = _mm256_sra_epi32(ref1, height_subsample_vector);
        ref_offset1 = _mm256_mullo_epi32(src_pitch_vector, temp_ref1); // packed DWORD multiplication
        break;
    case 3:
        temp_ref1 = _mm256_sra_epi32(ref1, width_subsample_vector);
        ref_offset1 = _mm256_sll_epi32(temp_ref1, pixel_step_shift_bits);
        break;
    case 4:
        temp_ref1 = _mm256_sra_epi32(ref1, height_subsample_vector);
        ref_offset1 = _mm256_mullo_epi32(src_pitch_vector, temp_ref1);
        temp_ref1 = _mm256_sra_epi32(ref1, width_subsample_vector);
        ref_offset2 = _mm256_sll_epi32(temp_ref1, pixel_step_shift_bits);
        break;
    case 2:
        // ref2: bit 8-15
        // similar to above
        __m256i ref2;
        ref2 = info_block;
        ref2 = _mm256_slli_epi32(ref2, 16); // << 16
        ref2 = _mm256_srai_epi32(ref2, 24); // >> 24

        __m256i ref1_fix, ref2_fix;
        // ref_px = src_pitch * info.ref2 + info.ref1;
        ref1_fix = _mm256_sra_epi32(ref1, width_subsample_vector);
        ref2_fix = _mm256_sra_epi32(ref2, height_subsample_vector);
        ref_offset1 = _mm256_mullo_epi32(src_pitch_vector, ref2_fix); // packed DWORD multiplication
        ref_offset1 = _mm256_add_epi32(ref_offset1, _mm256_sll_epi32(ref1_fix, pixel_step_shift_bits));

        // ref_px_2 = info.ref2 - src_pitch * info.ref1;
        ref1_fix = _mm256_sra_epi32(ref1, height_subsample_vector);
        ref2_fix = _mm256_sra_epi32(ref2, width_subsample_vector);
        ref_offset2 = _mm256_mullo_epi32(src_pitch_vector, ref1_fix); // packed DWORD multiplication
        ref_offset2 = _mm256_sub_epi32(_mm256_sll_epi32(ref2_fix, pixel_step_shift_bits), ref_offset2);
        break;
    default:
        abort();
    }
    ...
    if (info_data_stream){
        _mm256_store_si256((__m256i*)info_data_stream, ref_offset1);
        info_data_stream += parallel_width * 2;

        if (sample_mode == 2 || sample_mode == 4) {
            _mm256_store_si256((__m256i*)info_data_stream, ref_offset2);
            info_data_stream += parallel_width * 2;
        }
    }
...
static __forceinline __m256i generate_blend_mask_high(__m256i a, __m256i b, __m256i threshold)
{
    __m256i diff1 = _mm256_subs_epu16(a, b);
    __m256i diff2 = _mm256_subs_epu16(b, a);

    __m256i abs_diff = _mm256_or_si256(diff1, diff2);

    __m256i sign_convert_vector = _mm256_set1_epi16( (short)0x8000 );

    __m256i converted_diff = _mm256_sub_epi16(abs_diff, sign_convert_vector);

    __m256i converted_threshold = _mm256_sub_epi16(threshold, sign_convert_vector);

    // mask: if threshold >= diff, set to 0xff, otherwise 0x00
    // note that this is the opposite of low bitdepth implementation
    return _mm256_cmpgt_epi16(converted_threshold, converted_diff);
}


template<int sample_mode, bool blur_first>
static __m256i __forceinline process_pixels_mode12_high_part(__m256i src_pixels, __m256i threshold_vector, __m256i change, const __m256i& ref_pixels_1, const __m256i& ref_pixels_2, const __m256i& ref_pixels_3, const __m256i& ref_pixels_4)
{	
    __m256i use_orig_pixel_blend_mask_12, use_orig_pixel_blend_mask_34;
    __m256i avg_12, avg_34;
    __m256i dst_pixels;
    ...
    if (true)
    {
        if (!blur_first)
        {
            ...
            use_orig_pixel_blend_mask_12 = _mm256_and_si256(
        ...
        avg_12 = _mm256_avg_epu16(ref_pixels_1, ref_pixels_2);
    }
    ...
    if (process_34)
    {
        if (!blur_first)
        {
            use_orig_pixel_blend_mask_34 = _mm256_and_si256(
        ...
        avg_34 = _mm256_avg_epu16(ref_pixels_3, ref_pixels_4);
    }
    ...
    if (sample_mode == 2)
    {
        ...
        if (!blur_first)
            use_orig_pixel_blend_mask_12 = _mm256_and_si256(use_orig_pixel_blend_mask_12, use_orig_pixel_blend_mask_34 );
        avg_12 = _mm256_subs_epu16(avg_12, _mm256_set1_epi16(1));
        avg_12 = _mm256_avg_epu16(avg_12, avg_34);
    }
    ...
    if (sample_mode == 4)
    {
        ...
        __m256i dst_pixels_12 = _mm256_blendv_epi8(src_pixels, avg_12, use_orig_pixel_blend_mask_12);
        __m256i dst_pixels_34 = _mm256_blendv_epi8(src_pixels, avg_34, use_orig_pixel_blend_mask_34);
        dst_pixels = _mm256_avg_epu16(dst_pixels_12, dst_pixels_34);
    }
    else
    {
        ...
        dst_pixels = _mm256_blendv_epi8(src_pixels, avg_12, use_orig_pixel_blend_mask_12);
    }

    __m256i sign_convert_vector = _mm256_set1_epi16((short)0x8000);

    // convert to signed form, since change is signed
    dst_pixels = _mm256_sub_epi16(dst_pixels, sign_convert_vector);

    // saturated add
    dst_pixels = _mm256_adds_epi16(dst_pixels, change);

    // convert back to unsigned
    dst_pixels = _mm256_add_epi16(dst_pixels, sign_convert_vector);
    return dst_pixels;
}

template<int sample_mode, bool blur_first, int dither_algo>
static __m256i __forceinline process_pixels(
    __m256i src_pixels_0, 
    __m256i threshold_vector, 
    const __m256i& change_1, 
    const __m256i& ref_pixels_1_0,
    const __m256i& ref_pixels_2_0,
    const __m256i& ref_pixels_3_0,
    const __m256i& ref_pixels_4_0,
    const __m256i& clamp_high_add,
    const __m256i& clamp_high_sub,
    const __m256i& clamp_low,
    bool need_clamping,
    int row,
    int column,
    void* dither_context)
{
    __m256i ret = process_pixels_mode12_high_part<sample_mode, blur_first>
        (src_pixels_0, 
         threshold_vector, 
         change_1, 
         ref_pixels_1_0, 
    ...
    int height_in_pixels)
{
    ...
    case LOW_BIT_DEPTH:
        {
            pixels = _mm256_srli_epi16(pixels, 8);
            pixels = _mm256_packus_epi16(pixels, pixels);
            _mm256_storel_epi64((__m256i*)dst, pixels);
            return 8;
            break;
        }
    case HIGH_BIT_DEPTH_INTERLEAVED:
        pixels = _mm256_srl_epi16(pixels, downshift_bits);
        _mm256_store_si256((__m256i*)dst, pixels);
        return 16;
        break;
    default:
        abort();
...
static __m256i load_m256(const unsigned char *ptr)
{
    if (aligned)
    {
...
static __m256i __forceinline read_pixels(
    ...
    __m128i upsample_shift)
{
    ...
    case LOW_BIT_DEPTH:
        {
            ret = _mm256_cvtepu8_epi16(_mm_load_si128((__m128i*)ptr));
            return ret;
        }
        break;
    case HIGH_BIT_DEPTH_INTERLEAVED:
        ret = load_m256<aligned>(ptr);
        break;
    default:
        abort();
    }
    ret = _mm256_sll_epi16(ret, upsample_shift);
    return ret;
}
...
static __m256i __forceinline load_reference_pixels(
    __m128i shift,
    const unsigned short src[parallel_width])
{
    __m256i ret = _mm256_load_si256((const __m256i*)src);
    ret = _mm256_sll_epi16(ret, shift);
    return ret;
}
...
static void __forceinline read_reference_pixels(
    ...
    __m256i& ref_pixels_1_0,
    __m256i& ref_pixels_2_0,
    __m256i& ref_pixels_3_0,
    __m256i& ref_pixels_4_0)
{
...
static void __cdecl _process_plane_avx2_impl(const process_plane_params& params, process_plane_context* context)
{
    ...
    __m256i src_pitch_vector = _mm256_set1_epi32(params.src_pitch);
           
    __m256i threshold_vector = _mm256_set1_epi16(params.threshold);
    ...
    __m256i clamp_high_add = _mm256_setzero_si256();
    __m256i clamp_high_sub = _mm256_setzero_si256();
    __m256i clamp_low = _mm256_setzero_si256();
    if (need_clamping)
    {
        clamp_low = _mm256_set1_epi16((short)params.pixel_min);
        clamp_high_add = _mm256_sub_epi16(_mm256_set1_epi16((short)0xffff), _mm256_set1_epi16((short)params.pixel_max));
        clamp_high_sub = _mm256_add_epi16(clamp_high_add, clamp_low);
    }
    ...
    const int info_cache_block_size = ((sample_mode == 2 || sample_mode == 4) ? parallel_width * 8 : parallel_width * 4);
...
#define READ_REFS(data_stream, inp_mode) read_reference_pixels<sample_mode, dither_algo, inp_mode>( \
    ...
    #undef PROCESS_INFO_BLOCK
            ...
            change_1 = _mm256_load_si256((__m256i*)grain_buffer_ptr);

            __m256i dst_pixels = process_pixels<sample_mode, blur_first, dither_algo>(
                                     src_pixels, 
                                     threshold_vector,
                                     change_1, 
                                     ref_pixels_1_0, 
...
static void process_plane_avx2_impl_stub1(const process_plane_params& params, process_plane_context* context)
{
    switch (params.output_mode)
    {
    case LOW_BIT_DEPTH:
        _process_plane_avx2_impl<sample_mode, blur_first, dither_algo, aligned, LOW_BIT_DEPTH>(params, context);
        break;
    case HIGH_BIT_DEPTH_INTERLEAVED:
        _process_plane_avx2_impl<sample_mode, blur_first, dither_algo, aligned, HIGH_BIT_DEPTH_INTERLEAVED>(params, context);
        break;
    default:
        abort();
    }
...
template<int sample_mode, bool blur_first, int dither_algo>
static void __cdecl process_plane_avx2_impl(const process_plane_params& params, process_plane_context* context)
{
    if ( ( (intptr_t)params.src_plane_ptr & (AVX2_PLANE_ALIGNMENT - 1) ) == 0 && (params.src_pitch & (AVX2_PLANE_ALIGNMENT - 1) ) == 0 )
    {
        process_plane_avx2_impl_stub1<sample_mode, blur_first, dither_algo, true>(params, context);
    } else {
        process_plane_avx2_impl_stub1<sample_mode, blur_first, dither_algo, false>(params, context);
    }
}
*/
