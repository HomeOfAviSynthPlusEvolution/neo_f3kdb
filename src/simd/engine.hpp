...
{
    alignas(T_SSE4::align)
    unsigned short tmp_1[T_SSE4::width_16];
    alignas(T_SSE4::align)
    unsigned short tmp_2[T_SSE4::width_16];
    alignas(T_SSE4::align)
    unsigned short tmp_3[T_SSE4::width_16];
    alignas(T_SSE4::align)
    unsigned short tmp_4[T_SSE4::width_16];
    ...
    for (int i = 0; i < T_SSE4::width_16; i++)
    {
        int d1 = *(int*)(info_data_start + 4 * i);
        int d2 = *(int*)(info_data_start + 4 * (i + i / T_SSE4::width_32 * T_SSE4::width_32));
        int d3 = *(int*)(info_data_start + 4 * (i + i / T_SSE4::width_32 * T_SSE4::width_32 + T_SSE4::width_32));
        switch (sample_mode)
        {
        case 0:
            tmp_1[i] = read_pixel<input_mode>(src_px_start, i_fix + d1);
            break;
        case 1:
        case 3:
            tmp_1[i] = read_pixel<input_mode>(src_px_start, i_fix + d1);
            tmp_2[i] = read_pixel<input_mode>(src_px_start, i_fix + -d1);
            break;
        case 2:
        case 4:
            tmp_1[i] = read_pixel<input_mode>(src_px_start, i_fix + d2);
            tmp_2[i] = read_pixel<input_mode>(src_px_start, i_fix + -d2);
            tmp_3[i] = read_pixel<input_mode>(src_px_start, i_fix + d3);
            tmp_4[i] = read_pixel<input_mode>(src_px_start, i_fix + -d3);
            break;
        }
        i_fix += i_fix_step;
    }
    ...
    // ref_pixels_1_0 = T_SSE4::_max_epi8(ref_pixels_1_0, ref_pixels_2_0);
}
...
#ifdef SIMD_AVX2
#define READ_REFS(data_stream, inp_mode) \
    do { \
    __m128i tmp1l, tmp2l, tmp3l, tmp4l; \
    __m128i tmp1h, tmp2h, tmp3h, tmp4h; \
    read_reference_pixels<sample_mode, dither_algo, inp_mode>( \
                    params, \
    ...
    read_reference_pixels<sample_mode, dither_algo, inp_mode>( \
                    params, \
                    upsample_to_16_shift_bits, \
                    src_px + T_SSE4::width_16 * (params.input_mode == HIGH_BIT_DEPTH_INTERLEAVED ? 2 : 1), \
                    data_stream, \
                    tmp1h, \
                    tmp2h, \
                    tmp3h, \
                    tmp4h); \
    ref_pixels_1_0 = _mm256_set_m128i(tmp1h, tmp1l); \
    ref_pixels_2_0 = _mm256_set_m128i(tmp2h, tmp2l); \
    ref_pixels_3_0 = _mm256_set_m128i(tmp3h, tmp3l); \
    ref_pixels_4_0 = _mm256_set_m128i(tmp4h, tmp4l); \
    } while (0)
#else
#define READ_REFS(data_stream, inp_mode) \
    read_reference_pixels<sample_mode, dither_algo, inp_mode>( \
