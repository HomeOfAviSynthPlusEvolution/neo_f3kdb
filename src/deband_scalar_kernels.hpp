...
namespace neo_f3kdb::deband_scalar {
...
inline int avg4(int pixel1, int pixel2, int pixel3, int pixel4) {
    ...
    int avg2_value = avg2(pixel3, pixel4);
    if (avg1 > 0) {
        avg1 -= 1;
    }
...
template <bool kBlurFirst>
inline int process_mode4(int src_px, int ref_v1, int ref_v2, int ref_h1, int ref_h2, int threshold) {
...
int process_pixel(
    ...
    if constexpr (kSampleMode == 1) {
        ...
        const int ref1 = upsample(src_base[std::clamp(row - ref_y, 0, height - 1) * src_stride + col], input_depth);
        const int ref2 = upsample(src_base[std::clamp(row + ref_y, 0, height - 1) * src_stride + col], input_depth);
        ...
        const int ref1 = upsample(src_base[std::clamp(row - ref_y1, 0, height - 1) * src_stride + std::clamp(col - ref_x1, 0, width - 1)], input_depth);
        const int ref2 = upsample(src_base[std::clamp(row - ref_y2, 0, height - 1) * src_stride + std::clamp(col + ref_x2, 0, width - 1)], input_depth);
        const int ref3 = upsample(src_base[std::clamp(row + ref_y1, 0, height - 1) * src_stride + std::clamp(col + ref_x1, 0, width - 1)], input_depth);
        const int ref4 = upsample(src_base[std::clamp(row + ref_y2, 0, height - 1) * src_stride + std::clamp(col - ref_x2, 0, width - 1)], input_depth);
        ...
        const int ref_y = info.ref1 >> height_subsamp;
        const int ref_x = info.ref1 >> width_subsamp;
        const int ref_v1 = upsample(src_base[std::clamp(row - ref_y, 0, height - 1) * src_stride + col], input_depth);
        const int ref_v2 = upsample(src_base[std::clamp(row + ref_y, 0, height - 1) * src_stride + col], input_depth);
        ...
        const int ref_y = info.ref1 >> height_subsamp;
        const int ref_x = info.ref1 >> width_subsamp;
        const int ref_h1 = upsample(src_base[std::clamp(row - ref_y, 0, height - 1) * src_stride + col], input_depth);
        const int ref_h2 = upsample(src_base[std::clamp(row + ref_y, 0, height - 1) * src_stride + col], input_depth);
        ...
        return process_mode5(src_px, ref_h1, ref_h2, ref_w1, ref_w2, params.threshold, params.threshold1, params.threshold2);
    } else {
